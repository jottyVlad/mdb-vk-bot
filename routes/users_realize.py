...
@bp.on.message_handler(AccessForAllRule(), Registered(), text=["/профиль", "/profile", "/п"], lower=True)
async def profile_message(message: Message, user: Optional[User] = None):
    ...
    await make_profile_photo(user)
    files = {'photo': open(f"{user.user_id}.jpeg", "rb")}
