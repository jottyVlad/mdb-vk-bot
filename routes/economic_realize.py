...
async def payouts():
        ...
        time_unix = time.mktime((datetime.datetime.now()).timetuple()) - 20
        ...
        for user in users:
            ...
            await User.get(user_id=user['user_id'], peer_id=user['peer_id']).update(coins=user['coins']+work.salary, job_lp=datetime.datetime.now())
